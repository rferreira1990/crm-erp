...
<head>
    ...
    <style>
        ...
        .totals-box { width: 36%; margin-left: auto; margin-top: 14px; }
        .totals-box table { width: 100%; border-collapse: collapse; }
        .totals-box td { padding: 5px 8px; border-bottom: 1px solid #ececec; }
        .totals-box td:first-child { font-weight: bold; text-align: right; width: 62%; }
        .totals-box td:last-child { text-align: right; width: 38%; }
        .totals-box tr:last-child td { border-bottom: none; background: #1f6fa6; color: #fff; font-weight: bold; }
...
@php
    ...
    $minRows = 8;
    $fillerRows = max(0, $minRows - $items->count());
@endphp
        ...
        <table class="items-table">
            <thead>
                <tr>
                    ...
                    <th style="width: 12%;" class="text-right">Preco Un.</th>
                    <th style="width: 6%;" class="text-right">Dsc</th>
            ...
            <tbody>
                @forelse($items as $line)
                    @php
                        $itemCode = $line->item?->code ?: ($line->purchaseRequestItem?->item?->code ?: 'MANUAL');
                        $itemDescription = $line->description ?: ($line->purchaseRequestItem?->description ?: '-');
                        $unitCode = $line->item?->unit?->code ?: ($line->purchaseRequestItem?->item?->unit?->code ?: ($line->unit_snapshot ?: '-'));
                    @endphp
                    <tr>
                        ...
                        <td>
                            ...
                            @if($line->supplier_item_reference)
                                <div style="color:#666;">Ref: {{ $line->supplier_item_reference }}</div>
                            @endif
                        </td>
                        <td>
                            {{ $itemDescription }}
                            @if($line->notes)
                                <div style="color:#666;">Obs.: {{ $line->notes }}</div>
                            @endif
                        </td>
                        <td class="text-right">{{ number_format((float) $line->qty, 3, ',', '.') }}</td>
                ...
                @empty
                    <tr><td colspan="8" style="text-align:center;color:#666;padding:18px 0;">Sem linhas nesta encomenda preparada.</td></tr>
                @endforelse

                @for($i = 0; $i < $fillerRows; $i++)
                    <tr class="items-filler-row"><td colspan="8"></td></tr>
                @endfor
            </tbody>
        </table>
        ...
        <div class="totals-box">
            <table>
                <tr>
                    ...
                    <td>{{ number_format((float) $order->subtotal_amount, 2, ',', '.') }}</td>
                </tr>
            </table>
        </div>

        @if(!empty($order->notes))
            <div class="notes-section"><strong>Notas:</strong><br>{{ $order->notes }}</div>
        @endif
