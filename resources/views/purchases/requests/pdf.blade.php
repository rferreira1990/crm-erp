...
<head>
    ...
    <style>
        ...
        .draft-watermark { position: fixed; top: 42%; left: 50%; transform: translate(-50%, -50%) rotate(-32deg); font-size: 72px; font-weight: bold; color: rgba(180, 180, 180, 0.18); letter-spacing: 8px; z-index: 0; white-space: nowrap; }
...
<body>
    @php
        ...
        $minRows = 8;
        $fillerRows = max(0, $minRows - $purchaseRequest->items->count());
    @endphp

    <div class="document">
        @if($isDraft)<div class="draft-watermark">RASCUNHO</div>@endif
        ...
        <div class="content-layer">
            ...
            <table class="items-table">
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{{ $purchaseRequest->statusLabel() }}</td>
                    </tr>
                </tbody>
            </table>
            ...
            <table class="items-table">
                ...
                <tbody>
                    ...
                    @empty
                        <tr><td colspan="5" class="no-lines">Sem linhas no pedido de cotacao.</td></tr>
                    @endforelse

                    @for($i = 0; $i < $fillerRows; $i++)
                        <tr class="items-filler-row"><td colspan="5"></td></tr>
                    @endfor
                </tbody>
            </table>

            @if(!empty($purchaseRequest->notes))
                <div class="notes-section"><strong>Notas finais:</strong><br>{{ $purchaseRequest->notes }}</div>
            @endif

            <div class="notice-box">Este documento destina-se apenas a pedido de cotacao e nao inclui precos, descontos ou totais.</div>
